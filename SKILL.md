...
**Ask ALL questions together** so the user can answer everything at once. If the environment provides a structured question UI, use it; otherwise ask in one concise chat message:
...
2. **View each image** — Use the available image-inspection tool for attached images or local file paths. If image viewing is unavailable, inspect filenames/metadata and state the limitation.
3. **Evaluate** — For each: what it shows, USABLE or NOT USABLE (with reason), what concept it represents, dominant colors
4. **Co-design the outline** — Curated images inform slide structure alongside text. This is NOT "plan slides then add images" — design around both from the start (e.g., 3 screenshots → 3 feature slides, 1 logo → title/closing slide)
5. **Confirm with the user**: "Does this slide outline and image selection look right?" Offer: Looks good / Adjust images / Adjust outline
...
Create `.frontend-slides/slide-previews/`, then save previews there (style-a.html, style-b.html, style-c.html). Each should be self-contained, ~50-100 lines, showing one animated title slide.
